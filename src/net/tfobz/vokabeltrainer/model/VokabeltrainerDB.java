...
public class VokabeltrainerDB {
	...
	/**
	 * Die Methode zeigt, wie die Klasse VokabeltrainerDB verwendet werden kann, um
	 * auf die
	 ...
	 * @param nummerFach
	 * @return null falls Fach oder Lernkartei nicht vorhanden sind oder
	 ...
	 * 
	 * @param nummerFach
	 * @return null falls Fach nicht vorhanden sind oder Datenbankfehler aufgetreten
	 *         ist
	 */
	...
	/**
	 * Liefert eine zuf�llige Karte aus dem �bergebenen Fach der �bergebenen
	 * Lernkartei. Es wird
	 * nicht abgespeichert, dass in diesem Fach heute gelernt wurde. Das Fach muss
	 * zur aktuellen
	 * Lernkartei geh�ren
	 * 
	 * @param nummerLernkartei
	 ...
	 *         wenn es keine Karte in diesem Fach gibt oder wenn ein Datenbankfehler
	 *         auftritt
	 */
	...
	/**
	 * Schiebt eine Karte ein Fach nach hinten und merkt sich, dass in der
	 * Lernkartei
	 * gelernt wurde
	 * 
	 * @param karte
	 * @return 0 falls erfolgreich<br>
	 *         -1 falls Karte nicht abgespeichert oder Datenbankfehler aufgetreten
	 *         ist<br>
	...
	/**
	 * Schiebt die Karte ins erste Fach der Lernkartei und merkt sich, dass in der
	 * Lernkartei
	 * gelernt wurde
	 * 
	 * @param karte
	 * @return 0 falls erfolgreich<br>
	 *         -1 falls Karte noch nicht abgespeichert wurde oder Datenbankfehler
	 *         aufgetreten ist
	 */
	...
	/**
	 * F�gt neues Fach zur aktuellen Lernkartei am Ende hinzu. Im Fach wird die
	 * Nummer gesetzt,
	 * welche vom Datenbanksystem vergeben wird. Im Fach wird gelerntAm auf das
	 * heutige
	 * Datum gesetzt falls im �bergebenen Objekt gelerntAm nicht gesetzt wurde.
	 * Wurde die
	 * Fachbeschreibung nicht gesetzt, so wird diese auf "Fach <Nummer in
	 * Lernkartei>" gesetzt
	 * 
	 * @param nummerLernkartei
	 * @param fach
	 * @return -1 falls Lernkartei nicht existiert oder falls Fach schon in Datebank
	 *         existiert
	 */
	public static int hinzufuegenFach(int nummerLernkartei, Fach fach) {
		...
		if (fach != null && fach.getNummer() == -1 && getLernkartei(nummerLernkartei) != null) {
			...
			try {
				...
				String sql = "INSERT INTO faecher(fbeschreibung, ferinnerung, fgelerntam, lnummer) " +
	...
	/**
	 * �ndert die Inhalte eines bestehenden Faches. Dabei k�nnen nur die
	 * Beschreibung,
	 * die Erinnerung und gelerntAm ge�ndert werden
	 * 
	 * @param fach
	 * @return -1 falls Fach noch nicht in Datenbank exisitert oder
	 *         Datenbankfehler<br>
	 ...
	 *         dieser Beschreibung bereits existiert. In diesem Fall wird in
	 *         Beschreibung eine
		...
		if (lernkartei != null && lernkartei.getNummer() == -1) {
			...
			if (lernkartei.getFehler() != null)
				...
				try {
					...
					String sql = "INSERT INTO lernkarteien(lbeschreibung, lworteinsbeschreibung, " +
	 ...
	 *         aufgetreten ist<br>
	 *         -2 falls Validierungsfehler aufgetreten ist und falls Lernkartei mit
	 *         dieser
	 *         Beschreibung schon existiert. Im letzteren Fall wird f�r die
	 *         Beschreibung eine
	 *         entsprechende Fehlermeldung gesetzt
	 */
			...
			if (lernkartei.getFehler() != null)
				...
				try {
					...
					if (lernkartei.getBeschreibung() == null ||
						...
						sqlBeschreibung = "'" + lernkartei.getBeschreibung() + "'";
					String sql = "UPDATE lernkarteien " +
	...
	public static int loeschenLernkartei(int nummerLernkartei) {
		...
		try {
			con = getConnection();
			stmt = con.createStatement();
			String sql = "DELETE FROM lernkarteien " +
					"  WHERE lnummer = " + nummerLernkartei + ";";
			if (stmt.executeUpdate(sql) == 0)
				ret = -1;
			else {
	 ...
	 *         -5 falls dieselbe Karte in der Lernkartei in irgendeinem Fach schon
	 *         existiert<br>
	 */
			...
			if (karte.getFehler() != null)
				...
				if (faecher == null)
					...
					try {
						con = getConnection();
						stmt = con.createStatement();
						String sql = "SELECT lgrosskleinschreibung " +
								"  FROM lernkarteien " +
								"  WHERE lnummer = " + nummerLernkartei + ";";
						rs = stmt.executeQuery(sql);
						rs.next();
						...
						if (rs.getInt(1) != 0)
							...
							sql = "INSERT INTO karten(kworteins, kwortzwei, fnummer) " +
	 ...
	 * @param nummerLernkartei
	 * @param karte
	 * @return -1 falls Karte nicht in Datenbank ist oder Datenbankfehler
	 *         aufgetreten ist<br>
	 *         -2 falls Karte nicht vollst�ndig ist<br>
	 *         -4 falls Karte mit demselben Inhalten in irgendeinem Fach bereits
	 *         vorhanden ist
	 */
	...
	public static int loeschenAlleFaecher(int nummerLernkartei) {
		...
		if (!(getLernkartei(nummerLernkartei) == null)) {
			...
			try {
				con = getConnection();
				stmt = con.createStatement();
				String sql = "DELETE FROM faecher " +
						"  WHERE lnummer = " + nummerLernkartei + ";";
				stmt.executeUpdate(sql);
				con.commit();
				ret = 0;
	 ...
	 * Erinnerung abgelaufen ist. Eine Erinnerung l�uft ab, wenn die Differenz
	 * zwischen
	 * dem heutigen Datum und dem Datum an dem zuletzt gelernt wurde gr��er oder
	 * gleich der
	 * Erinnerung ist. Auch werden alle Lernkarteien zur�ck geliefert
	 * die F�cher enthalten in denen noch nie gelernt wurde. Ist das
	 * Erinnerungsintervall 0 so
	...
	public static List<Lernkartei> getLernkarteienErinnerung() {
		...
		try {
			con = getConnection();
			stmt = con.createStatement();
			String sql = "SELECT l.lnummer, l.lbeschreibung, " +
	...
	/**
	 * Liefert zur �bergebenen Lernkartei die F�cher bei denen die Erinnerung
	 * abgelaufen
	 * ist
	 * 
	 * @return null falls Lernkartei nicht existiert oder Datenbankfehler
	 *         aufgetreten
	 *         ist<br>
	 *         eine leere ArrayList wenn keine F�cher zur Lernkartei existieren bei
	 *         denen die
	...
	public static List<Fach> getFaecherErinnerung(int nummerLernkartei) {
		...
		if (getLernkartei(nummerLernkartei) != null) {
			...
			try {
				con = getConnection();
				stmt = con.createStatement();
				String sql = "SELECT f.fnummer, f.fbeschreibung, f.ferinnerung, " +
	...
	public static ArrayList<Karte> getKarten(int nummerFach) {
		...
		if (getFach(nummerFach) != null) {
			...
			try {
				con = getConnection();
				String sql = "SELECT k.knummer, kworteins, kwortzwei, l.lrichtung, l.lgrosskleinschreibung " +
						"  FROM karten k, faecher f, lernkarteien l " +
						"  WHERE k.fnummer = f.fnummer AND " +
						"    f.lnummer = l.lnummer AND " +
	 ...
	 * ebenfalls neu angelegt werden. Doppelt vorhandene Karten werden nicht
	 * importiert
	 * 
	 * @nummerLernkartei
	 * @param pfad
	...
	public static int importierenKarten(int nummerLernkartei, String pfad) {
		...
		if (l == null)
			ret = -3;
		else if (pfad != null && pfad.length() > 0) {
				...
				while (true) {
					...
					if (zeile == null)
						...
						if (!geloescht) {
							...
							stmt.executeUpdate(sql);
							if (anzahlSpalten == 2) {
								sql = "INSERT INTO faecher(lnummer) " +
						...
						if (anzahlSpalten == 3) {
							...
							if (fach == null) {
								fach = new Fach();
								String sql = "INSERT INTO faecher(lnummer) " +
										"  VALUES (" + nummerLernkartei + ");";
								if (stmt.executeUpdate(sql, Statement.RETURN_GENERATED_KEYS) == 0) {
									ret = -1;
									break;
	...
	/**
	 * Exportiert alle Karten der �bergebenen Lernkartei in eine Textdatei mit oder
	 * ohne F�cher.
	 ...
	 * @param pfad
	 * @param mitFaechern
	 * @return -1 falls Datenbankfehler oder Schreibfehler in Datei aufgetreten
	 *         ist<br>
	...
	public static int exportierenKarten(int nummerLernkartei, String pfad, boolean mitFaechern) {
		...
		if (getLernkartei(nummerLernkartei) == null)
			...
			try {
				...
				con = getConnection();
				stmt = con.createStatement();
				String sql = "SELECT kworteins, kwortzwei, fnummer " +
						"  FROM karten, faecher " +
	...
	public static Lernkartei getEinstellungenStandardLernkartei() {
		...
		try {
			...
			stmt = con.createStatement();
			String sql = "SELECT l.lnummer, l.lbeschreibung, l.lworteinsbeschreibung, l.lwortzweibeschreibung, " +
					"    l.lrichtung, l.lgrosskleinschreibung " +
	...
	/**
	 * Liefert zur�ck ob der Benutzer nur in Lernkarteien mit abgelaufener
	 * Erinnerung
	 * lernen m�chte
	 * 
	...
	public static boolean getEinstellungenLernkarteienMitErinnerung() {
		...
		try {
			con = VokabeltrainerDB.getConnection();
			stmt = con.createStatement();
			String sql = "SELECT elernkarteienmiterinnerung " +
					"  FROM einstellungen;";
			rs = stmt.executeQuery(sql);
			if (rs.next())
